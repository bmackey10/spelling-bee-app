          ...
          {spellingBees.map((spellingBee) => {
              ...
              return (
                <Link to={`/spelling-bee/${spellingBee.id}`}>
                  <button type="button" className="border-2 border-black bg-black hover:bg-bee-yellow text-white hover:text-black rounded-full px-8 py-2 text-sm font-semibold shadow-sm">Play: {formattedDate}</button>
                </Link>
              );
          })}
      </div>
